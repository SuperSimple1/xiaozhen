(function($) {
	$.fn.slide = function(options) {
		...
		//切换点列表
		function changeNavaList() {
			if (settings.nava) {
				var index = el.find('.active').attr("data-index");
				$(".navaListBox li a").removeClass("active").eq(index - 1).addClass("active");
			}
		};
		//自动切换
		function autoRun() {
		...
		$.fn.moveTo = function(index) {
			...
			var nowIndex = $(".navaListBox li a.active").attr('data-index');
			if (index < nowIndex) {
				moveWhere = 'left';
			} else {
				moveWhere = 'right';
			};
			$(".navaListBox li").eq(index - 1).children().addClass('active').parent().siblings().children().removeClass('active');
		...
		$.each($(settings.slideContainer), function(i) {
			...
			leftPos = leftPos + 100;
			if (settings.nava) {
				navalist += "<li style='cursor: pointer'><a data-index='" + (i + 1) + "'></a></li>";
			};
			$.each($(this).children(), function(i) {
				$(this).attr('offsetLeft', $(this).css('left'));
			});
		});
		$("<ul style='width:" + ($(settings.slideContainer).length * 20 + ($(settings.slideContainer).length - 1) * 5) + "px;margin-left:-" + ($(settings.slideContainer).length * 12 + (($(settings.slideContainer).length - 1) * 5)) / 2 + "px' class='navaListBox'>" + navalist + "</ul>").appendTo(el.parent());
		$(settings.slideContainer).eq(0).addClass("active");
		if (settings.nava) $(".navaListBox li a" + "[data-index=1]").addClass("active");
		$(".navaListBox li a").bind({
			click: function() {
				if (!isMove) {
					return;
				};
